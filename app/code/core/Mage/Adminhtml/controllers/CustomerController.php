...
class Mage_Adminhtml_CustomerController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        ...
        $old_emailid = $customerData['email'];
        $new_mailid=$data['account']['email'];
        
        if($group_id==4 && $old_emailid!=$new_mailid)
        {
			$db = Mage::getSingleton('core/resource')->getConnection('core_write');
			$db->exec("SET @@session.wait_timeout = 1200");
			$db->query("update admin_user set email='$new_mailid' where email='$old_emailid'");
        	$db->query("update merchant set email='$new_mailid' where email='$old_emailid'");
        }
