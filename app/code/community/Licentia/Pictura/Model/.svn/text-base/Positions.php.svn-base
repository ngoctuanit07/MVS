 ...
 */
class Licentia_Pictura_Model_Positions extends Mage_Core_Model_Abstract {

    const MYSQL_DATE = 'yyyy-MM-dd';

    protected function _construct() {
        $this->_init('pictura/positions');
    }

    public function getSelectedPositionsIds($bannerId) {

        return $this->getCollection()
                        ->addFieldToFilter('banner_id', $bannerId)
                        ->getAllIds('position_id');
    }
